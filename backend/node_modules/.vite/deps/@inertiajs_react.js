...
import { h as mergeConfig$1, m as isCancel, p as isAxiosError, v as axios } from "./axios-CJgHvgoJ.js";
...
var Router = class {
	...
	*/
	...
	visit(href, options = {}) {
		...
		if (prefetched) {
			progress$1.reveal(prefetched.inFlight);
			prefetchedRequests.use(prefetched, requestParams);
		} else {
			progress$1.reveal(true);
...
function handleStartEvent(event, delay) {
	if (!event.detail.visit.showProgress) progress$1.hide();
	const timeout = setTimeout(() => progress$1.start(), delay);
	document.addEventListener("inertia:finish", (e) => finish(e, timeout), { once: true });
}
function handleProgressEvent(event) {
	if (progress$1.isStarted() && event.detail.progress?.percentage) progress$1.set(Math.max(progress$1.getStatus(), event.detail.progress.percentage / 100 * .9));
}
function finish(event, timeout) {
	clearTimeout(timeout);
	if (!progress$1.isStarted()) return;
	if (event.detail.visit.completed) progress$1.finish();
	else if (event.detail.visit.interrupted) progress$1.reset();
	else if (event.detail.visit.cancelled) progress$1.remove();
}
...
function useForm(...args) {
	...
	const reset = (0, import_react.useCallback)((...fields) => {
		...
		else commitData(fields.filter((key) => has(defaults, key)).reduce((carry, key) => {
...
function usePoll(interval, requestOptions = {}, options = {
	...
	const latest = (0, import_react.useRef)(requestOptions);
	latest.current = requestOptions;
	const pollRef = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		pollRef.current = router.poll(interval, typeof requestOptions === "function" ? () => latest.current() : requestOptions, {
			...options,
			autoStart: options.autoStart ?? true
		});
...
function usePrefetch(options = {}) {
	...
	const [lastUpdatedAt, setLastUpdatedAt] = (0, import_react.useState)(cached?.staleTimestamp || null);
	const [isPrefetching, setIsPrefetching] = (0, import_react.useState)(inFlight !== null);
	const [isPrefetched, setIsPrefetched] = (0, import_react.useState)(cached !== null);
	(0, import_react.useEffect)(() => {
		const onPrefetchingListener = router.on("prefetching", (e) => {
			if (e.detail.visit.url.pathname === window.location.pathname) setIsPrefetching(true);
		});
...
//#endregion
export { App, Deferred_default as Deferred, Form_default as Form, Head_default as Head, InfiniteScroll_default as InfiniteScroll, Link_default as Link, WhenVisible_default as WhenVisible, config, createInertiaApp, http, progress, router3 as router, useForm, useFormContext, usePage, usePoll, usePrefetch, useRemember };
